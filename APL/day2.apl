...
⍝   it first applies ' '∘≠ to its argument, which returns a boolean vector where 0's
⍝    correspond to spaces
⍝   it then applies the identity function to its argument
⍝   it then applies ⊆ (partition) between the two results, which has the effect of
⍝    splitting its argument on spaces
⍝
